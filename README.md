...
| <code>searchSuggestionsUri(keyword:)</code> | GET | <code>/api/v4/search/suggest?q=...&magi=1</code> |
| <code>fetchSearchSuggestions(keyword:)</code> | GET | 解析搜索补全列表 |
| <code>searchHotUri()</code> | GET | <code>/api/v4/search/hot_search</code> |
| <code>fetchSearchHotItems(limit:)</code> | GET | 解析热搜列表 |
| <code>fetchSearchFilterGroups()</code> | GET | 解析搜索筛选配置 |
...
本项目仅供个人学习、技术研究与交流使用，严禁用于任何商业用途、营利活动或其他违法违规场景。

本项目完全免费并开源，不销售任何功能或服务，不设置付费内容，也不接受任何形式的打赏、赞助或捐赠。项目涉及的接口路径、参数及响应结构等资料均收集、整理自公开互联网和 GitHub 开源项目，仅用于技术学习与研究，不代表知乎官方，也未获得知乎官方授权或背书。

使用者应自行遵守所在地法律法规、相关平台协议及内容版权要求，不得利用本项目侵犯任何个人或组织的合法权益。因使用、修改或分发本项目产生的任何风险与责任由使用者自行承担。知乎相关名称、商标、接口、数据及内容的权利归其各自权利人所有；如权利人认为本项目存在不当内容，请通过 GitHub Issues 联系处理。
